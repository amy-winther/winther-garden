...
function renderStack() {
  ...
  doneSection.innerHTML = `
    ...
    <div class="done-swipe-hint">← swipe to reopen or change</div>
...
function renderGrid() {
  ...
  chipContainer.innerHTML = cats.map(c => {
    const count = c === "All" ? tasks.length : tasks.filter(t => t.data.category === c).length;
    return `<button class="filter-chip ${c === gridFilterCat ? "active" : ""}" data-cat="${c}">${c} (${count})</button>`;
  }).join("");
...
// ── localStorage pruning ─────────────────────────────────────
function pruneOldData() {
  const cutoff = CURRENT_YEAR - 1;
  Object.keys(localStorage).forEach(key => {
    if (!key.startsWith("wg_")) return;
    // formats: wg_YYYY_M_plant or wg_note_YYYY_M_plant
    const stripped = key.startsWith("wg_note_") ? key.slice(8) : key.slice(3);
    const year = parseInt(stripped.split("_")[0]);
    if (!isNaN(year) && year < cutoff) localStorage.removeItem(key);
  });
}
